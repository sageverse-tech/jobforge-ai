...
def search_jobs_with_claude(query: str, api_key: str, resume_context: str) -> list:
    client = anthropic.Anthropic(api_key=api_key)
    system = f"""You are a job search assistant. Search the web for real, current job postings based on the user's query.
...
{{
  ...
  "match": <integer 75-99 based on how well the resume fits>
}}
Resume context for matching: {resume_context[:1000]}...
...
def generate_cover_letter(job: dict, api_key: str, resume_text: str, user_name: str) -> str:
    ...
    response = client.messages.create(
        model="claude-sonnet-4-20250514",
        max_tokens=1000,
        messages=[{"role": "user", "content": f"""Write a professional cover letter for {user_name}.
...
Rules: 3 paragraphs max, confident tone, highlight relevant experience from the resume, sign as {user_name}, start with Dear Hiring Manager,"""}],
    )
...
with st.expander("⚙️ Step 1: Configure Your Profile (Required)", expanded=not st.session_state.resume_text):
    st.markdown("<div style='font-size:14px;font-weight:600;margin-bottom:10px;color:#79c0ff'>1. AI Brain Setup</div>", unsafe_allow_html=True)
    api_input = st.text_input("Anthropic API Key", type="password", placeholder="sk-ant-...", value=st.session_state.api_key)
    ...
    user_name_input = st.text_input("Your Full Name (for the cover letter signature)", value=st.session_state.user_name)
    ...
    if uploaded_file is not None:
        with st.spinner("Reading resume..."):
            ...
            st.success("✓ Resume successfully loaded into AI memory!")
...
with col_left:
    ...
    job_role = st.text_input("What role are you looking for?", placeholder="e.g. Frontend Developer, Data Analyst")
    location = st.text_input("Location", placeholder="e.g. Remote, Bangalore, India")
    filter_type = st.selectbox("Job Type", ["Internship", "Full-time", "Any"])
    ...
    if st.button("⚡ Search Live Jobs Now", disabled=not ready_to_search):
        all_jobs = []
        progress = st.progress(0, text="Scanning LinkedIn & Naukri...")
        
        # Create a dynamic search query based on user input
        type_str = "" if filter_type == "Any" else filter_type
        loc_str = f"in {location}" if location else ""
        query = f"{job_role} {type_str} jobs {loc_str} 2026 site:linkedin.com OR site:naukri.com OR site:internshala.com"

        try:
            results = search_jobs_with_claude(query, st.session_state.api_key, st.session_state.resume_text)
            all_jobs.extend(results)
        except Exception as ex:
            st.warning(f"Search error: {ex}")
    ...
    if not st.session_state.api_key or not st.session_state.resume_text:
        ...
        ⚠ Please complete Step 1 (API Key & Resume) to enable search</div>""", unsafe_allow_html=True)
    ...
    if st.session_state.jobs:
        ...
        <div style='display:flex;align-items:center;justify-content:space-between;margin:18px 0 10px'>
          <div style='font-size:13px;font-weight:600;color:#fff'>{len(jobs)} Match Found</div>
          <div style='font-size:11px;color:#6e7681;font-family:"JetBrains Mono",monospace'>scored against your resume</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        for i, job in enumerate(jobs):
            ...
            with st.expander(f"{'🟢' if score>=92 else '🟡' if score>=85 else '🟠'} {job.get('title','N/A')} — {job.get('company','N/A')}", expanded=False):
                ...
                if job.get("applyLink"):
                    ...
                    <a href="{job['applyLink']}" target="_blank" style="display:block;background:#0f1a2a;border:1px solid #1f3a5c;border-radius:8px;padding:10px;text-decoration:none;color:#79c0ff;font-size:12px;text-align:center;margin-bottom:10px">
                      ↗ Apply on {job.get('source','Website')}
                    </a>
                    """, unsafe_allow_html=True)

                if st.button(f"✍ Generate Custom Cover Letter", key=f"gen_{i}"):
                    with st.spinner("AI is analyzing your resume and writing the letter..."):
                        try:
...
with col_right:
    if st.session_state.selected and st.session_state.cover_letter:
        ...
        <div style='background:#0f1e35;border:1px solid #1f4a7a;border-radius:12px;padding:16px 18px;margin-bottom:16px'>
          <div style='font-size:14px;font-weight:700;color:#fff'>Generated for: {job.get('title')}</div>
          <div style='font-size:12px;color:#79c0ff;margin-top:4px'>{job.get('company')}</div>
        </div>
        """, unsafe_allow_html=True)

        st.markdown("""<div style='font-size:11px;color:#56d364;font-family:"JetBrains Mono",monospace;margin-bottom:8px'>✓ Smart Cover Letter Ready</div>""", unsafe_allow_html=True)
        st.text_area("Cover Letter", value=st.session_state.cover_letter, height=280, key="cover_display", label_visibility="collapsed")

        email_draft = f"Subject: Application for {job.get('title')} — {st.session_state.user_name}\n\n{st.session_state.cover_letter}\n\n---\n📎 Attachment: Resume.pdf"
        st.markdown("""<div style='font-size:11px;color:#79c0ff;font-family:"JetBrains Mono",monospace;margin:14px 0 8px'>📧 Email Draft (Copy & Paste)</div>""", unsafe_allow_html=True)
        ...
        <div style='background:#0a0f1a;border:1px solid #161b22;border-radius:12px;padding:40px 24px;text-align:center;margin-top:40px'>
          ...
          <div style='font-size:14px;font-weight:600;color:#fff;margin-bottom:8px'>Your Magic Assistant</div>
          <div style='font-size:12px;color:#6e7681;line-height:1.7'>
            Once you find a job on the left, click <br><b>Generate Cover Letter</b> and AI will write<br> it perfectly based on your uploaded resume.
          </div>
        </div>
        """, unsafe_allow_html=True)
...
<div style='text-align:center;font-size:10px;color:#21262d;font-family:"JetBrains Mono",monospace;padding:10px'>
  JobForge AI v2.0 • Public Edition
</div>
""", unsafe_allow_html=True)
